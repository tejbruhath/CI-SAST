...
from . import config, crypto, executor
...
def create_pr(fix_id: str, user_id=None) -> dict:
    ...
    try:
        base = scm.default_branch(repo, token=user_token)
...
def create_batch_pr(repo_full_name: str, user_id=None) -> dict:
    ...
    fixes = list(
        ...
        .filter(finding__scan__requested_by_id=user_id,
                finding__scan__target__icontains=repo_full_name,
                status__in=[FixSuggestion.APPROVED, FixSuggestion.EDITED],
                pr_status=FixSuggestion.PR_NONE)
        .order_by("-finding__severity_score", "created_at"))
    if not fixes:
        return {"status": "empty", "count": 0}

    scan = fixes[0].finding.scan
    repo = scm.parse_repo(scan.target)
    if repo is None:
    ...
    branch = f"sentriq/fixes-{timezone.now():%Y%m%d-%H%M%S}"
    for fx in fixes:
        fx.pr_status = FixSuggestion.PR_CREATING
        fx.branch = branch
        fx.pr_error = ""
    ...
    try:
        ...
        lines = "\n".join(
            ...
            f"`{fx.finding.file}` — {fx.explanation}" for fx in fixes)
        ...
        for fx in fixes:
            fx.pr_status = FixSuggestion.PR_OPEN
            fx.pr_url = url
        ...
        detail = getattr(exc, "stderr", "") or str(exc)
        for fx in fixes:
            fx.pr_status = FixSuggestion.PR_FAILED
